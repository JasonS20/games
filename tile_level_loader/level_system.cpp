...
void LevelSystem::load_level(const std::string& path, float tile_size) {
  ...
  for (int i = 0; i < buffer.size(); ++i) {
    ...
    case 's':
      ...
      _start_position = get_tile_position(sf::Vector2i(static_cast<int>(x),static_cast<int>(h)));
      break;
    case 'e':
...
void LevelSystem::build_sprites() {
    ...
    for (size_t y = 0; y < LevelSystem::get_height(); ++y) {
        for (size_t x = 0; x < LevelSystem::get_width(); ++x) {
            ...
            s->setPosition(get_tile_position(pos));
            s->setSize(sf::Vector2f(_tile_size, _tile_size));
            s->setFillColor(get_color(get_tile(pos)));
...
LevelSystem::Tile LevelSystem::get_tile_at(sf::Vector2f v) {
  ...
  return get_tile(sf::Vector2i(static_cast<int>((v - _offset).x / _tile_size),static_cast<int>((v - _offset).y / _tile_size)));
}
...
std::vector<sf::Vector2i> LevelSystem::find_tiles(LevelSystem::Tile type) {
  ...
  for (size_t i = 0; i < _width * _height; ++i) {
    if (_tiles[i] == type) {
      v.push_back(sf::Vector2i(static_cast<int>(i % _width),static_cast<int>(i / _width)));
    }
  }
